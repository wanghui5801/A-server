...
const SSHTerminalModal: React.FC<SSHTerminalModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in p-2 sm:p-4">
      <div className="bg-[#1C1C1C] rounded-lg shadow-2xl w-full max-w-5xl h-[95vh] sm:h-[85vh] border border-gray-800/30 modal-enter">
        <div className="flex justify-between items-center h-12 px-3 sm:px-6 border-b border-gray-800/30">
          <div className="flex items-center gap-2 sm:gap-3 animate-slide-down overflow-x-auto pb-2 sm:pb-0">
            <h2 className="text-base sm:text-lg font-medium text-gray-100 whitespace-nowrap">SSH Terminal</h2>
            <div className="flex items-center gap-2">
              <div className="px-2 sm:px-3 py-1 bg-[#252525] rounded text-xs sm:text-sm text-gray-300 font-medium transition-all duration-300 hover:bg-opacity-80 whitespace-nowrap">
                {hostname}
              </div>
              <div className="px-2 sm:px-3 py-1 bg-[#252525] rounded text-xs sm:text-sm text-gray-300 font-medium transition-all duration-300 hover:bg-opacity-80 whitespace-nowrap">
                {publicIp || clientIp}
        ...
        <div className="h-[calc(100%-3rem)] flex flex-col overflow-hidden p-1">
          {!isAuthenticated ? (
            <div className="min-h-full flex items-center justify-center bg-[#1C1C1C] p-4 sm:p-6 animate-fade-in">
              <div className="w-full max-w-md p-4 sm:p-8 bg-[#252525] rounded-lg shadow-lg animate-fade-in backdrop-blur-sm bg-opacity-95 border border-gray-800/30">
                ...
                <form onSubmit={handleConnect} className="space-y-4">
                  <div className="group">
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-3 sm:px-4 py-2 bg-[#1C1C1C] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500/50 transition-all duration-300 border border-gray-800/50 group-hover:border-gray-700/50 text-sm sm:text-base"
                  ...
                  {error && (
                    <div className="text-red-400 text-xs sm:text-sm bg-red-500/10 px-3 py-2 rounded-lg border border-red-500/20 animate-shake">
                  ...
                  <button
                    type="submit"
                    className={`w-full py-2 sm:py-2.5 rounded-lg font-medium transition-all duration-300 text-sm sm:text-base ${
                      isConnecting
                        ? 'bg-green-500/70 cursor-not-allowed'
                        : 'bg-green-500 hover:bg-green-400 hover:shadow-lg active:scale-[0.98]'
                    }`}
                    disabled={isConnecting}
                  >
                    {isConnecting ? (
            ...
            <div 
              ref={terminalRef} 
              className="flex-1 bg-[#1E1E1E] terminal-container"
              style={{ minHeight: 0 }}
            />
          )}
        </div>
